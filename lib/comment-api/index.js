...
app.get('/comment/my-comments', restrict, function (req, res) {
  ...
  api.comment.all({topicId: req.query.id, userId: req.user.id}, function (err, comments) {
    if (err) return _handleError(err, req, res);
...
app.get('/comment/sidecomments', function (req, res) {
  ...
  api.comment.all({topicId: req.query.id, context: 'paragraph'}, function (err, comments) {
    if (err) return _handleError(err, req, res);
...
app.post('/comment', restrict, function (req, res) {
  if (!req.query.topicId) {
    log('Request didn\'t include `topicId` parameter');
    res.error(400);
  }

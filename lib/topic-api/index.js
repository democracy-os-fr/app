...
var topicListKeys = [
  'id topicId title mediaTitle status open closed public draft deleted forum frameId',
...
app.get('/topic/:id/sidecomments', function (req, res) {
  ...
  api.comment.getSideComments(req.params.id, null, function (err, comments) {
    ...
    var keys = [
      ...
      'author.id author.fullName author.displayName author.avatar author.country',
...
app.post('/topic/:id/vote', restrict, function (req, res) {
  ...
  .vote(
    ...
    function (err, topic) {
      ...
      if(( config.maintenanceMode != 'notifications' )){
...
app.post('/topic/create', restrict, hasAccess, function (req, res, next) {
  ...
  api.topic.create(req.body, function (err, topic) {
    ...
    var keys = [
      ...
      'publishedAt deletedAt votable bodyTruncationText links author authorUrl frameId'
    ].join(' ');
    res.json(exposeTopic(topic, req.user));
  });
});

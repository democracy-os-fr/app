...
app.post('/identity/enable', restrict, function (req, res, next) {
  var citizen = req.user;
  ...
  function onresponse(err, citizen) {
    if (!err) {
...
app.post('/identity/disable', restrict, function (req, res, next) {
  var citizen = req.user;

  citizen.netidEnabledAt = null;
  citizen.save(function (err) {
    if (err) return res.send(500);
    res.send(200);
  });

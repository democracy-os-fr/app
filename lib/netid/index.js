...
NetIdRequest.prototype.end = function(fn) {
  ...
  var self = this;
  req.end(function (err, res) {
    if (res.body && res.body.error == 'invalid_grant') {
      log('token has expired, re-authenticate and try again');
      manager.state('expired');
      manager.authenticate();
      return fn(new Error('Acess token has expired'), res);
    } else {
      fn(err, res);
    }
  });

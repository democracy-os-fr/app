...
page('/law/:id', citizen.optional, load, function(ctx, next) {
  ...
  comments.render('.main-content');
  comments.initialize();

  if (citizen.id) {
    classes(o('.main-content')).add('logged');
  }
  if (options.voted) {
    classes(o('.main-content')).add('voted');
  }
  if (options.closed) {
    classes(o('.main-content')).add('closed');
  }
  if (options['no census']) {
    classes(o('.main-content')).add('no-census');
  }

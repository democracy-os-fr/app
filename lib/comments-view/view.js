...
CommentsView.prototype.validate = function(data) {
  ...
  if (data.text.length > 4096) {
    errors.push('Comment is limited to 4096 characters');
  };
  return errors;
}
  ...
  .end(function(err, res) {
    if (err || !res.ok) {
      if (!res || res.status == 500) {
        netidTemplates.render('identity-verification-error', 'div.help-text.form-errors');
        return;
      } else if (res.status == 403) {
        netidTemplates.render('identity-not-verified', 'div.help-text.form-errors');
        return;
      }
    }

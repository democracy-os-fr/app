...
var netidTemplates = require('netid-templates');
...
CommentsView.prototype.errors = function(errors) {
  var span = o('div.help-text.form-errors', this.el);
  errors = errors || [];
...
CommentsView.prototype.onlike = function(ev) {
  ...
  var originalCount = parseInt(counter.innerHTML, 10) || 0;
  var count = disliked ? 2 : (liked ? 0 : 1);
  counter.innerHTML = originalCount + count;

  var self = this;
  this.switchOff();

  var netidEnabled = config['netid enabled'];

  if (!netidEnabled || (netidEnabled && citizen.netid)) {
    request
    .post('/api/comment/:id/upvote'.replace(':id', id))
    .end(function(err, res) {

      if (!netidTemplates.handleError(err, res, div)) {
        classes(target).remove('selected');
        counter.innerHTML = originalCount;
      }

      return self.switchOn();
    });
  } else {
    netidTemplates.render('citizen-not-enabled-netid', div);
  }
}
...
CommentsView.prototype.ondislike = function(ev) {
  ...
  var counter = o('.comment-counter', comment);
  var originalCount = parseInt(counter.innerHTML, 10) || 0;
  var count = liked ? 2 : (disliked ? 0 : 1);
  counter.innerHTML = originalCount - count;
...
CommentsView.prototype.post = function(data) {
  ...
  if (!netidEnabled || (netidEnabled && citizen.netid)) {
    ...
    .send({ comment: data })
    .end(function(err, res) {
      netidTemplates.handleError(err, res, 'div.help-text.form-errors');
    ...
    netidTemplates.render('citizen-not-enabled-netid', 'div.help-text.form-errors');
  }
}
...
CommentsView.prototype.onconfirmremove = function(ev) {
  ...
  request
    ...
    .end(function(err, res) {
      ...
      classes(comment).remove('remove');
      var messageEl = o('.message', comment);

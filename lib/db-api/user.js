...
exports.search = function search(query, fn) {
  ...
  .select('firstName lastName fullName email avatar profilePictureUrl notifications country')
  .exec(function(err, users) {
    if (err) {
      log('Found error: %j', err);
      return fn(err);
...
exports.getByEmail = function search(email, fn) {
  ...
  .select('firstName lastName fullName email avatar profilePictureUrl notifications emailValidated country')
  .exec(function(err, user) {
    if (err) {
...
exports.expose.confidential = function(user){
  return expose('id firstName lastName displayName email avatar staff notifications locale country')(user);
}
...
exports.expose.ordinary = function(user){
  return expose('id fullName displayName avatar notifications locale country')(user);
}

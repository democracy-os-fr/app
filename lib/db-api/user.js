...
exports.search = function search(query, fn) {
  ...
  .select('firstName lastName fullName email avatar notifications country')
  .exec(function(err, users) {
    if (err) {
      log('Found error: %j', err);
      return fn(err);
...
exports.getByEmail = function search(email, fn) {
  ...
  .select('firstName lastName fullName email avatar notifications emailValidated country')
  .exec(function(err, user) {
    if (err) {
...
exports.expose.confidential = function(user){
  return expose('id firstName lastName email avatar staff notifications locale country')(user);
}
...
exports.expose.ordinary = function(user){
  return expose('id fullName avatar notifications locale country')(user);
}

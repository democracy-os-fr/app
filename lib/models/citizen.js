...
var CitizenSchema = new Schema({
  ...
  , netidEnabledAt: { type: Date, default: null }
	, profiles:  {
...
CitizenSchema.plugin(passportLocalMongoose, {
  usernameField: 'email',
  userExistsError: 'email.already.taken'
});
...
/**
 * Get `netid` from netidEnabledAt date
 *
 * @return {Boolean} netid
 * @api public
 */

CitizenSchema.virtual('netid').get(function() {
  return this.netidEnabledAt != null;
});

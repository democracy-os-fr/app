...
var bus = require('bus');
var participantTemplate = require('./participant');
var box = require('./box');
...
Participants.prototype.fetch = function() {
  ...
  .end(function(err, res) {
    ...
    res.body.forEach(function (v) {
      var participant = reactive(render(participantTemplate), {participant: v});
      ps.push(participant.el);
    });
    view.participants = ps;
    view.view = reactive(render(box), { participants: view.participants });
    view.state('loaded');
  });
}
...
Participants.prototype.onvote = function() {
  ...
  var participant = reactive(render(participantTemplate), {participant: citizen});
  this.participants.push(participant.el);
};

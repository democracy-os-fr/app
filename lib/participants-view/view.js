...
function View(participants) {
  ...
  this.boxTpl = render(box);
  this.participantTpl = render(participant);
...
View.prototype.render = function(el) {
  ...
  this.view.on('view-more', this.more.bind(this));
};
...
View.prototype.onvote = function() {
  if (~this.participants.indexOf(citizen) || ~this.participants.indexOf(citizen.id)) return;
...
View.prototype.more = function(ev) {
  var btn = o('a.view-more', this.view.el);
  classes(btn).add('hide');

  var hiddens = o.all('a.participant-profile.hide', this.view.el) || [];

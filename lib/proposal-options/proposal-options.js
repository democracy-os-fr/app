...
function ProposalOptions (proposal, citizen) {
  ...
  var locals = { proposal: proposal, citizen: citizen, t: t };
  var positives = locals.positives = proposal.upvotes || [];
  var negatives = locals.negatives = proposal.downvotes || [];
  var neutrals = locals.neutrals = proposal.abstentions || [];
  var census = locals.census = neutrals.concat(negatives).concat(positives) || [];
  var participants = locals.participants = proposal.participants || [];
  this.closed = locals.closed = proposal.closingAt && +new Date(proposal.closingAt) < Date.now();
  this.voted = locals.voted = citizen && ~census.indexOf(citizen.id);
  this['no census'] = census.length == 0;

  this.el = domify(options(locals));
...
ProposalOptions.prototype.vote = function(ev) {
  ...
  if (this.citizen.id) {
    ...
    classes(o('.main-content')).add('login');
  }
...
ProposalOptions.prototype.changevote = function(ev) {
  ...
  classes(o('.main-content')).toggle('change');
};

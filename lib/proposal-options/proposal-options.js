...
ProposalOptions.prototype.vote = function(ev) {
  ...
  var target = ev.delegateTarget || closest(ev.target, '[data-proposal]', true)
  ...
  if (this.citizen.id) {
    ...
    this.post(
      ...
      function (err, res) {
        if (err || !res.ok) {
          function renderNetIdResponse(template) {
            var el = render.dom(template);
            var messagesEl = o('.direct-vote .messages', this.el);
            empty(messagesEl).appendChild(el);
          }

          if (!res || res.status == 500) {
            renderNetIdResponse(netidTemplates.identityVerificationError);
            return log('Failed cast %s for %s with error: %j', value, id, err || res.error);
          } else if (res.status == 403) {
            renderNetIdResponse(netidTemplates.identityNotVerified);
            return;
          }
        } 
    ...
    classes(o('.proposal-options p.text-mute.hide', this.el)).remove('hide');
  }
